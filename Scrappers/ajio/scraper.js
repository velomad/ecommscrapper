	...
	for (var i = 0; i < loopArry.length; i++) {
		for (var j = 0; j < loopArry[i].length; j++) {
			for (var p = 1; p <= pagesToScrape; p++) {
				var URL = `${url}/api/category/${Object.values(loopArry[i][j])}?fields=SITE&currentPage=${p}&pageSize=45&format=json&query=%3Arelevance&sortBy=relevance&gridColumns=3&advfilter=true`;
				let data = [];
